// Two clients replaying the same seed and move list MUST land on the same
// position with the same teleport log, or multiplayer desyncs.
...
for (let round = 0; round < 40; round++) {
  ...
  // replay from scratch (the reconnecting client)
  ...
  // different seed diverges (sanity that the seed actually matters)
  if (ucis.length > 4) {
    const other = new ChaosMatch(seed + 'x');
    other.shuffleIfDue();
    // not asserted strictly (could coincide), just recorded
  }
}
